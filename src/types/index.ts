export type TaskStatus = 'todo' | 'queued' | 'briefing' | 'planning' | 'declaring' | 'running' | 'architecting' | 'review' | 'done' | 'blocked';
...
/** The Formic surface which initiated an agent invocation. */
export type UsageScope = 'task' | 'assistant' | 'messaging';

interface BaseUsageEvent {
  /** `${sessionId}:${messageId}` (or an OpenCode step-part equivalent). */
  id: string;
  timestamp: string;
  /** 'brief' | 'plan' | 'execute' | 'quick' | 'reflection' | 'assistant' */
  step: string;
  agentType: AgentType;
  source: UsageSource;
  /** The provider session that emitted this token record. */
  sessionId: string;
  model: string;
  inputTokens: number;
  outputTokens: number;
...
/** Token usage generated by a workflow task. */
export interface TaskUsageEvent extends BaseUsageEvent {
  scope: 'task';
  taskId: string;
}

/** Token usage generated outside a workflow task. */
export interface NonTaskUsageEvent extends BaseUsageEvent {
  scope: 'assistant' | 'messaging';
  /** Stable assistant-session or messaging-conversation identity. */
  scopeId: string;
  taskId?: never;
}

/** Token usage attributed to one agent transcript message. */
export type UsageEvent = TaskUsageEvent | NonTaskUsageEvent;
...
export interface Task {
  ...
  /** Number of execution retry attempts for this task */
